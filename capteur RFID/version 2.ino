  ...
  if (!modeGestion) {
    ...
    if (success) {
      if (memcmp(uid, idAdmin, 4) == 0) {
        ...
        publishMQTT("Mode gestion activé");
        delay(1000);
        return;
      }
    ...
    if (success) {
      if (memcmp(uid, idAdmin, 4) == 0 && memcmp(uid, dernierUID, 4) != 0) {
        ...
        publishMQTT("Mode gestion désactivé");
        delay(1000);
        return;
      }
      ...
      for (uint8_t i = 0; i < nombreCartes; ++i) {
        if (memcmp(cartesAcceptees[i], uid, 4) == 0) {
          ...
          publishMQTT("Carte retirée de la liste.");
      ...
      if (nombreCartes < MAX_CARTES) {
        ...
        publishMQTT("Carte ajoutée à la liste.");
        delay(1000);
        return;
      } else {
